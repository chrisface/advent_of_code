...
byebug
last_winning_board = boards[winners.last[:board_index]]
winning_number = winners.last[:winning_number]

score = last_winning_board.flatten.select{ |c| c.checked == false }.map(&:number).reduce(&:+) * winning_number
puts score